...
- The non-interactive defaults for pagers, editors, git, apt, opam, and pip now live in the session's fixed environment (`unattendedGuestEnv`) rather than in whatever `bash_init` inherited, so every fork of a checkpoint and every recursive snapshot carries the same values.
- Session teardown unmounts by mount depth rather than by path length, so sibling mount points with long names no longer sort ahead of their own submounts.
- `bash_init` no longer answers a client that disconnects before sending a request with `request_too_large`; that status is now reserved for genuine protocol violations.
